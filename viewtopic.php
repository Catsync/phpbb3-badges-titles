...
while ($row = $db->sql_fetchrow($result))
{
	...
	if (!isset($user_cache[$poster_id]))
	{
		...
		else
		{
			...
			// MERYL TODO: Add Badges/Title info to user cache here?
			$user_cache[$poster_id]['badges-display'] = get_user_displayed_badges($poster_id);
			$user_cache[$poster_id]['title-display'] = get_user_displayed_title($poster_id);
		}
	}
}
$db->sql_freeresult($result);
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	// MERYL TODO: add badge/title data to postrow here
		'U_HAS_BADGES'	=> ($user_cache[$poster_id]['badges-display'] == '' ? false : true),
		'U_BADGES_DISPLAY'		=> $user_cache[$poster_id]['badges-display'],
		'U_HAS_TITLE'	=> ($user_cache[$poster_id]['title-display'] == '' ? false : true),
		'U_TITLE_DISPLAY'	=> $user_cache[$poster_id]['title-display'],
